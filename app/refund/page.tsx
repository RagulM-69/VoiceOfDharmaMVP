...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getLegalPage('refund').catch(() => null)
  
  const title = page?.seo?.metaTitle ?? 'Refund Policy | Voice of Dharma Foundation'
  const description = page?.seo?.metaDescription ?? 'Refund Policy for the Voice of Dharma Foundation.'
...
export default async function RefundPage() {
  const [page, settings] = await Promise.all([
    getLegalPage('refund').catch(() => null),
