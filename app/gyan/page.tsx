...
export const metadata: Metadata = {
  title: 'Gyaan — The Path of Knowledge | Voice of Dharma',
  description: 'Explore the principle of Gyaan: the pursuit of wisdom and understanding as taught in the Bhagavad Gita.',
}

export const revalidate = 3600

export default async function GyanPage() {
  const content = await getSiteContent('gyan')
  const footerContent = await getSiteContent()

  const heroTitle   = getContent(content, 'gyan', 'hero',    'title',    'Gyaan')
  const heroSub     = getContent(content, 'gyan', 'hero',    'subtitle', 'The Path of Knowledge')
  const mainIntro   = getContent(content, 'gyan', 'main',    'intro',    '')
  const gitaIntro   = getContent(content, 'gyan', 'gita',    'intro',    '')
  const gitaQuote   = getContent(content, 'gyan', 'gita',    'quote',    '')
  const gitaRef     = getContent(content, 'gyan', 'gita',    'ref',      '')
  const modernH     = getContent(content, 'gyan', 'modern',  'heading',  '')
  const modernB     = getContent(content, 'gyan', 'modern',  'body',     '')
  const visionH     = getContent(content, 'gyan', 'vision',  'heading',  '')
  const visionB     = getContent(content, 'gyan', 'vision',  'body',     '')
  const serviceH    = getContent(content, 'gyan', 'service', 'heading',  '')
  const serviceB    = getContent(content, 'gyan', 'service', 'body',     '')
      ...
      <main>
        ...
        <section className="relative pt-32 pb-24 text-center overflow-hidden" style={{ background: '#0A1F44' }}>
          <div className="absolute inset-0">
            <Image
              src="/images/gyaan-manuscripts.png"
              alt="Gyaan — ancient wisdom and knowledge"
        ...
        {/* Service CTA — with manuscripts image */}
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <SectionWrapper delay={0.1}>
              <div className="relative w-full h-64 md:h-80 rounded-2xl overflow-hidden mb-10">
                <Image
                  src="/images/gyaan-manuscripts.png"
                  alt="Ancient manuscripts — the path of knowledge"
                  fill
