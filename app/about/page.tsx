...
export default async function AboutPage() {
  ...
  const platforms = [
    { label: 'YouTube', desc: 'Watch discourses and dharmic teachings', href: socialLinks.youtube ?? 'https://youtube.com/@voice_of_dharma', color: '#FF0000',
    ...
    { label: 'Instagram', desc: 'Follow for daily spiritual reflections', href: socialLinks.instagram ?? 'https://www.instagram.com/voiceofdharma.foundation', color: '#E1306C',
    ...
    { label: 'Facebook', desc: 'Join our community page', href: socialLinks.facebook ?? 'https://www.facebook.com/profile.php?id=61584215592924', color: '#1877F2',
    ...
    { label: 'WhatsApp', desc: 'Connect via our community channel', href: socialLinks.whatsapp ?? 'https://whatsapp.com/channel/0029Vb7QURUDDmFQStDDMg2v', color: '#25D366',
      ...
      <main>
        ...
        <section className="relative pt-32 pb-24 overflow-hidden" style={{ background: '#0A1F44' }}>
          <div className="absolute inset-0">
            <NextImage src="/images/about-dawn-landscape.png" alt="About Voice of Dharma — Varanasi ghats at dawn" fill className="object-cover opacity-20" priority sizes="100vw" />
          ...
          <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <SectionWrapper>
              ...
              {subheading && <p className="text-gray-300 text-xl leading-relaxed max-w-2xl mx-auto">{subheading}</p>}
            </SectionWrapper>
          </div>
        </section>
        ...
        {quote1 && (
          <div className="py-10 border-y border-amber-400/20" style={{ background: 'rgba(10,31,68,0.97)' }}>
            <div className="max-w-3xl mx-auto px-4 text-center">
              <p className="font-garamond text-xl md:text-2xl italic text-white/90 leading-relaxed">&ldquo;{quote1}&rdquo;</p>
              {quote1ref && <p className="text-amber-400/80 text-sm mt-3 tracking-wider">— {quote1ref}</p>}
            </div>
          </div>
        )}
          ...
          <section className="py-20 bg-cream">
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
              <SectionWrapper>
                ...
                <h2 className="font-garamond text-3xl md:text-4xl font-semibold text-krishna-blue mb-8">About the Foundation</h2>
                {mainBody.split('\n\n').map((para, i) => (
        ...
        {quote2 && (
          <div className="py-10 border-y border-amber-400/20" style={{ background: 'rgba(10,31,68,0.97)' }}>
            <div className="max-w-3xl mx-auto px-4 text-center">
              <p className="font-garamond text-xl md:text-2xl italic text-white/90 leading-relaxed">&ldquo;{quote2}&rdquo;</p>
              {quote2ref && <p className="text-amber-400/80 text-sm mt-3 tracking-wider">— {quote2ref}</p>}
            </div>
          </div>
        )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                  {platforms.map((platform) => (
                    <a key={platform.label} href={platform.href} target="_blank" rel="noopener noreferrer"
                      className="flex flex-col items-center p-6 rounded-xl bg-white shadow-sm border border-gray-100 hover:shadow-md transition-all hover:-translate-y-1 group">
                      <div className="mb-3 transition-transform group-hover:scale-110" style={{ color: platform.color }}>{platform.svg}</div>
                      <p className="font-semibold text-gray-800 text-sm">{platform.label}</p>
                      <p className="text-gray-400 text-xs mt-1 text-center">{platform.desc}</p>
                    </a>
                  ))}
                </div>
              )}
              <div className="flex justify-center mt-10">
                <Link href="/contact" className="inline-flex items-center gap-2 px-10 py-4 rounded-full font-semibold text-white text-base transition-all duration-300 hover:-translate-y-1 hover:shadow-xl" style={{ background: 'linear-gradient(135deg, #0A1F44, #1a3a6e)' }}>
                  ✉️ Get in Touch
                </Link>
              </div>
            </SectionWrapper>
